    ...
    internal abstract class AreaOp
    {
        internal abstract class CagOp : AreaOp
        {
            ...
            public override int Classify(Edge e)
            {
                ...
                _inResult = newClass;
                return (newClass ? EtagEnter : EtagExit);
            }

            public override int GetState()
            {
                return (_inResult ? RstagInside : RstagOutside);
            }
        ...
        internal class NzWindOp : AreaOp
        {
            ...
            public override int Classify(Edge e)
            {
                ...
                // assert(e.getCurveTag() == CtagLeft);
                int newCount = _count;
                int type = (newCount == 0 ? EtagEnter : EtagIgnore);
                newCount += e.GetCurve().GetDirection();
                _count = newCount;
                return (newCount == 0 ? EtagExit : type);
            }

            public override int GetState()
            {
                return ((_count == 0) ? RstagOutside : RstagInside);
            }
        }
        ...
        internal class EoWindOp : AreaOp
        {
            ...
            public override int Classify(Edge e)
            {
                // Remember: the right curves should be an empty set with this op...
                // assert(e.getCurveTag() == CtagLeft);
                bool newInside = !_inside;
                _inside = newInside;
                return (newInside ? EtagEnter : EtagExit);
            }

            public override int GetState()
            {
                return (_inside ? RstagInside : RstagOutside);
            }
        }
        ...
        public const int CtagLeft = 0;
        public const int CtagRight = 1;

        /* Constants to classify edges */
        public const int EtagIgnore = 0;
        public const int EtagEnter = 1;
        public const int EtagExit = -1;

        /* Constants used to classify result state */
        public const int RstagInside = 1;
        public const int RstagOutside = -1;
        ...
        public ArrayList Calculate(ArrayList left, ArrayList right)
        {
            ...
            AddEdges(edges, left, CtagLeft);
            AddEdges(edges, right, CtagRight);
        ...
        private ArrayList PruneEdges(ArrayList edges)
        {
            ...
            while (left < numedges)
            {
                ...
                // assert(getState() == AreaOp.RstagOutside);
                if (GetState() != RstagOutside)
                {
                    for (cur = left; cur < right; cur++)
                    {
                        e = edgelist[cur];
